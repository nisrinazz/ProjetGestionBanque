...
public class ServiceAdmin implements IServiceAdmin, Verifiable {
	...
	public TableauDeBord calculerEtAfficherStatistiques() {
		...
		tabDeBord.setNomClientLePlusRiche(maxSoldeMinSolde().get("max")!=null ? maxSoldeMinSolde().get("max").getPropriétaire().getNomComplet() : null);
		return tabDeBord;
	}
	@Override
	...
	public List<Compte> trierComptesParNomPropriétaire() {
		List<Compte> list = compteDAO.findAll();
		Collections.sort(list,(c1 , c2) -> c1.getPropriétaire().getNomComplet().compareToIgnoreCase(c2.getPropriétaire().getNomComplet()));
		return list ; 
	}
